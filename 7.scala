...
final case class Interpreter(
  ...
  def tillIO: Either[Interpreter, Interpreter] = Instruction(code(pos)).apply(pos) match {
    case Halt(_) => Left(this)
    case Input(_) if stdIn.isEmpty => Right(this)
    case o @ Output(_, _) => Right(o runOn this)
    case instruction => instruction.runOn(this).tillIO
  }
...
final case class AmplifierLoop private (interpreters: Seq[Interpreter], haltedMachines: Set[Int]) {
  private def feedImpl(input: List[Int], interpreterIdx: Int): Int = {
    val idx = interpreterIdx % interpreters.length
    if (haltedMachines contains idx)
      feedImpl(input, idx + 1)
    else {
      val interpreter = interpreters(idx)
      val withInput = interpreter.copy(
        stdIn = input ++ interpreter.stdIn
      )
      withInput.tillIO match {
        case Left(halted) if haltedMachines.size + 1 == interpreters.length => halted.stdOut.head
        case Left(halted) =>
          copy(interpreters = interpreters.updated(idx, halted), haltedMachines = haltedMachines + idx)
            .feedImpl(halted.stdOut, idx + 1)
        case Right(waitsForInput) =>
          copy(interpreters = interpreters.updated(idx, waitsForInput)).feedImpl(waitsForInput.stdOut, idx + 1)
      }
    }
  }
  def feed(input: Int): Int = feedImpl(input :: Nil, 0)
}

object AmplifierLoop {
  def apply(code: Code, phases: Seq[Int]): AmplifierLoop = {
    val interpreters = phases map { phase => Interpreter(code, stdIn = phase :: Nil) }
    val (interpretersWithPhases, halted) = interpreters.zipWithIndex.map { case (interpreter, idx) =>
      interpreter.tillIO match {
        case Left(halted) => halted -> Some(idx)
        case Right(paused) => paused -> None
      }
    }.unzip

    new AmplifierLoop(
      interpretersWithPhases,
      halted.flatten.toSet
    )
  }
}
...
object Day7 extends App {
  ...
  def run(inputParams: List[Int], usingPhase: Int): Interpreter =
    Interpreter(rawData, stdIn = usingPhase :: inputParams).fullRun


  def part1: Int = (for {
    phasesPermutation <- (0 to 4).permutations
  } yield phasesPermutation.foldLeft(0) { (input, phase) =>
    run(input :: Nil, phase).stdOut.head
  }).max

  def part2: Int = (for {
    loopPhasesPermutation <- (5 to 9).permutations
    ampLoop = AmplifierLoop(rawData, loopPhasesPermutation)
  } yield ampLoop feed 0).max
